...
<?php
    $team_info = "SELECT * FROM project3 WHERE email = '$email' OR mem1_email = '$email' OR mem2_email ='$email'";
...
<?php }?>
<?php
    $ret_stat = "SELECT * FROM statement WHERE project_no ='$id_no' AND project_type = 'Project-III'";
...
<?php
  $query = "SELECT * FROM documents WHERE project_no = '$id_no' and def_type = 'proposal' AND project_type = 'Project-III'";
...
{
  $query = "SELECT * FROM documents WHERE project_no = '$id_no' and def_type = 'final' AND project_type = 'Project-III'";
  $run_sql = mysqli_query($conn,$query);
  while($row=mysqli_fetch_array($run_sql))
  {
    $f_path =$row[6];
...
<?php
  ...
  if(isset($_POST['profsubmit']))
  {
    ...
    if(mysqli_query($conn,$insert_document))
    {
      ...
      $sql = "UPDATE project3 SET final = 'pending...' WHERE id = '$id_no'";
      ...
      $msg = "Team no #$id has submitted final document for Project-III.";
