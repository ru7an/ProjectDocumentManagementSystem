...
?>
 <center>
  <table style="width: 60%">
    <?php
      $check_email="SELECT * FROM project1 WHERE email='$email_user1' OR mem1_email='$email_user1' OR mem2_email='$email_user1'";
      $check=mysqli_query($conn,$check_email);
...
if(isset($_POST['p1_add']))
{
  ...
  $check_email="SELECT * FROM project1 WHERE email='$email'";
  $check=mysqli_query($conn,$check_email);
  ...
  if(mysqli_num_rows($check)>0)
  {
    if ($mem1 == "")
    {
      $update_team="UPDATE project1 SET mem1_email = '$member', mem1_name = '$name_mem' WHERE email='$email'";
      if(mysqli_query($conn,$update_team))
      {
        $msg = "$em_name has added you to his team.";
        $add_note = "INSERT INTO notificat (email_note,notify,project_type,status_note,sender) VALUES ('$member','$msg','Project-I','stall','$email')";
    ...
    elseif ($mem1!="" && $mem2=="")
    {
      $update_team="UPDATE project1 SET mem2_email = '$member', mem2_name = '$name_mem' WHERE email='$email'";
      if(mysqli_query($conn,$update_team))
      {
        $msg = "$em_name has added you to his team.";
        $add_note = "INSERT INTO notificat (email_note,notify,project_type,status_note,sender) VALUES ('$member','$msg','Project-I','stall','$email')";
  ...
  {
    $create_team = "INSERT INTO project1 (email, email_name, mem1_email, mem1_name, mem2_email, mem2_name, proposal,mid , final,sup_email, sup_name,college) VALUES ('$email', '$em_name', '$member', '$name_mem', '', '','none', 'none', 'none', 'none', 'none','$college')";
...
if(isset($_POST['sendnoneteam']))
{
  ...
  if(mysqli_num_rows($check)>0)
  {
    if ($mem1 == "")
    {
      $update_team="UPDATE project1 SET mem1_email = 'none', mem1_name = 'none' WHERE email='$email'";
      if(mysqli_query($conn,$update_team))
      {
        echo "<script>window.open('../Project-I','_self')</script>";
      }
    ...
    elseif ($mem1!="" && $mem2=="")
    {
      $update_team="UPDATE project1 SET mem2_email = 'none', mem2_name = 'none' WHERE email='$email'";
      if(mysqli_query($conn,$update_team))
      {
        echo "<script>window.open('../Project-I','_self')</script>";
      }
  ...
  {
    $create_team = "INSERT INTO project1 (email, email_name, mem1_email, mem1_name, mem2_email, mem2_name, proposal,mid , final,sup_email, sup_name,college) VALUES ('$email', '$em_name', '$member', '$name_mem', 'none', 'none','none', 'none', 'none', 'none', 'none','$college')";
    if(mysqli_query($conn,$create_team))
    {
      echo "<script>window.open('../Project-I','_self')</script>";
    }
